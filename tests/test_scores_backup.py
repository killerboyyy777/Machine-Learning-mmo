...
with tempfile.TemporaryDirectory() as tmp:
    ...
    old_file, old_scores = srv.SCORES_FILE, srv.SCORES
    srv.SCORES_FILE = db
    try:
        ...
        srv.SCORES["a"] = 1
        srv.save_scores()
        assert json.load(open(db)) == {"a": 1}
        assert not os.path.exists(db + ".1")
        srv.SCORES["b"] = 2
        srv.save_scores()
        assert json.load(open(db)) == {"a": 1, "b": 2}
        assert json.load(open(db + ".1")) == {"a": 1}
        # third save rolls generations: .2 appears, .1 holds latest-old
        srv.SCORES["c"] = 3
        srv.save_scores()
        assert json.load(open(db + ".1")) == {"a": 1, "b": 2}
        assert json.load(open(db + ".2")) == {"a": 1}
        print("ROTATE_OK")
